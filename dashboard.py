...
### CODE FOR MATPLOTLIB/SNS VIZ
#def price_plot(symbol):
#  df = pd.DataFrame(stock_data[symbol].Close)
#  df['Date'] = df.index
  #fig = plt.figure()
#  plt.fill_between(df.Date, df.Close, color='skyblue', alpha=0.3)
#  sns.lineplot(df.Date, df.Close, color='skyblue')
#  plt.xticks(rotation=55)
#  ax = plt.gca()
#  ax.spines['top'].set_visible(False)
 # ax.spines['right'].set_visible(False)
#  plt.title((df_selected_sector.loc[df_selected_sector['Symbol'] == symbol, 'Security'].iloc[0] +" "+"("+symbol+")"), fontweight='bold')
